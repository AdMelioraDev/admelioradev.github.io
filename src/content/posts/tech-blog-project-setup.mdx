...
<ProjectExplorer 
  ...
  next.config.ts // Next.js 설정 파일 ## Next.js의 설정 파일로, 빌드 옵션, 경로 재작성, 환경 변수 등을 구성한다.
  next-env.d.ts // Next.js 타입 정의 파일 ## Next.js의 TypeScript 타입 정의 파일로, Next.js 관련 타입을 TypeScript에 제공한다.
  package.json // 프로젝트 의존성 정보 ## 프로젝트의 메타데이터와 의존성 패키지 목록을 포함하는 npm 설정 파일이다.
  package-lock.json // 의존성 버전 잠금 파일 ## npm이 생성하는 파일로, 정확한 의존성 버전을 기록하여 일관된 설치를 보장한다.
  postcss.config.mjs // PostCSS 설정 파일 ## PostCSS 설정 파일로, CSS 전처리 및 후처리 플러그인(Tailwind CSS 등)을 구성한다.
  README.md // 프로젝트 설명 파일 ## 프로젝트에 대한 설명, 설치 방법, 사용법 등을 담은 마크다운 문서이다.
  tsconfig.json // TypeScript 설정 파일 ## TypeScript 컴파일러 설정 파일로, TypeScript 코드를 JavaScript로 변환하는 방법을 정의한다.
  `}
